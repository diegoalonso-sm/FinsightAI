...
class NewsExtractor:
    ...
    """
    Abstract base class for news extraction using crawling and LLM-based content parsing.

    This class defines the interface for extracting article URLs and extracting structured
    content using a generated schema via a language model.
    ...
    async def extract_urls(self, crawler, url: str, prefix: str) -> List[str]:
        ...
        """
        Extracts sanitized internal article URLs from a base page.

        :param crawler: The async crawler instance used for navigation.
        :type crawler: AsyncWebCrawler

        :param url: The base URL to extract links from.
        :type url: str

        :param prefix: Prefix used to filter valid internal article links.
        :type prefix: str

        :return: A list of cleaned internal URLs matching the prefix.
        :rtype: List[str]
    ...
    async def extract_content(self, crawler, urls: List[str], model: str, sample_html: str, llm_query: str, schema_model: Path = None) -> List[dict]:

        """
        Extracts structured content from the given URLs using a JSON schema generated by the LLM.

        :param crawler: The async crawler instance used to load and extract page content.
        :type crawler: AsyncWebCrawler

        :param urls: List of article URLs to process.
        :type urls: List[str]

        :param model: The identifier of the LLM model provider (e.g., "openai/gpt-4o-mini").
        :type model: str

        :param schema_model: The identifier of the LLM model used for schema generation.
        :type schema_model: str

        :param sample_html: HTML sample used to instruct the LLM in schema generation.
        :type sample_html: str

        :param llm_query: Prompt to instruct the LLM which fields to extract and how.
        :type llm_query: str

        :return: List of structured JSON-like dictionaries containing article data.
        :rtype: List[dict]
        ...
        content_extractor = StructuredExtractor(
            crawler=crawler,
            extraction_strategy=JsonCssExtractionStrategy(schema=schema)
        )

        extracted_data = await content_extractor.extract(
            urls=urls,
        )
    ...
    @staticmethod
    def _load_schema(model: str, sample_html: str, llm_query: str, schema_model: Path) -> dict:

        """
        Load a schema from a file, or generate it using LLM if the file is missing, empty, or invalid.
        If generated, save the new schema to the given file path.

        :param schema_model: Path to the schema file.
        :type schema_model: str

        :param sample_html: HTML sample to generate schema if needed.
        :type sample_html: str

        :param llm_query: Query to guide LLM-based schema generation.
        :type llm_query: str

        :param model: LLM provider model name.
        :type model: str
...
class YahooFinanceNewsExtractor(NewsExtractor):
    ...
    def __init__(self, duration_seconds: int, scroll_interval: int):
        ...
        :param duration_seconds: Total scrolling time in seconds for dynamic page loading.
    ...
    async def run(self) -> List[dict]:
        ...
        """
        Executes the full extraction process: Crawls Yahoo Finance news page for article links,
        then extracts structured content using an LLM-generated schema.

        :return: A list of structured dictionaries containing the extracted news data.
        :rtype: List[dict]
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            urls = await self.extract_urls(crawler=crawler, url=self.start_url, prefix=self.prefix)
            ...
            extracted_news = await self.extract_content(
                ...
                schema_model=Path(Path(__file__).parent / "schemas/yahoo_finance_schema.json"),
            )
    ...
    def _get_llm_query():
        ...
        """
        Returns a prompt for the LLM to generate the JSON schema.

        :return: Query prompt to guide the LLM in structuring extracted data.
        :rtype: str
...
async def usage_example() -> None:
    ...
    """
    Example usage of the YahooFinanceNewsExtractor class.

    This example:
    - Instantiates a YahooFinanceNewsExtractor with a 1-second scroll duration and 1-second scroll interval.
    - Launches an asynchronous extraction process that:
        - Scrolls and explores the Yahoo Finance news page.
        - Filters and sanitizes the extracted article links.
        - Extracts structured article data using a dynamic JSON schema generated by an LLM model.
    - Prints the structured extraction results in a formatted JSON output.

    Requirements:
    - An active internet connection.
    - Access to an LLM provider configured via environment variables (e.g., OpenAI API key if using `openai/gpt-4o-mini`).
    - Proper installation of `crawl4ai` and any other dependencies.
